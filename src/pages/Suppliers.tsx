...
export default function Suppliers() {
  ...
  const printLPO = () => {
    ...
    const rows = lpoItems.map((i) =>
      `<tr><td>${i.description}</td><td>${i.quantity}</td><td>${i.unit}</td><td>KES ${i.unit_price.toFixed(2)}</td><td>KES ${(i.quantity * i.unit_price).toFixed(2)}</td></tr>`
    ).join("");
    w.document.write(`
      <html><head><title>LPO ${lpoNumber}</title>
      ...
      <body>
      <div class="header">
        <div><h2>${currentOrg?.name || "Company"}</h2><p>LOCAL PURCHASE ORDER</p></div>
        <div style="text-align:right"><strong>LPO No: ${lpoNumber}</strong><br>Date: ${new Date().toLocaleDateString("en-KE")}<br>${lpoDeliveryDate ? `Delivery By: ${lpoDeliveryDate}` : ""}</div>
      </div>
      <p><strong>To:</strong> ${supplier.name}<br>${supplier.address || ""}<br>${supplier.phone || ""} ${supplier.email || ""}</p>
      ...
      ${lpoNotes ? `<p style="margin-top:12px"><strong>Notes:</strong> ${lpoNotes}</p>` : ""}
      <div class="sig">
        <div>Authorized Signatory</div>
        <div>Supplier Acknowledgment</div>
      </div>
      <div class="footer">This is a computer-generated document. Generated by SmartOps on ${new Date().toLocaleString("en-KE")}</div>
      </body></html>
    `);
    w.document.close();
    w.focus();
